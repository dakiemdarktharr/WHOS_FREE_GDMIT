...
The visual system uses a viewport-fixed liquid-chrome texture with animated iridescent lighting, beveled chrome Audiowide headings, Rajdhani controls, and glossy cel-shaded panels. Selected hours snap into a padlock state. Days with busy hours display a purple glint at 50% opacity over their existing color. Reduced-motion preferences retain static state feedback. The presentation file map and behavior contract are recorded in [../../../docs/ARCHITECTURE.md](../../../docs/ARCHITECTURE.md#current-frontend-presentation-2026-09-24). These visual changes do not migrate the legacy API described above.
